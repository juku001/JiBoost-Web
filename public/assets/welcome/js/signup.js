document.addEventListener('DOMContentLoaded', function () {
    ...
    function updateImage() {
        ...
        if (checkedRadio && userImage) {
            const imagePath = checkedRadio.dataset.image; // Use data-image attribute
            userImage.src = imagePath;

            // Save the selected user type in localStorage
            localStorage.setItem('userType', checkedRadio.value);
        }
    }
    ...
    if (savedUserType) {
        const savedRadio = document.querySelector(`input[name="user-type"][value="${savedUserType}"]`);
        if (savedRadio) {
            savedRadio.checked = true;
            updateImage(); // Ensure image updates on page load
        }
    } else {
        // If no saved selection, set a default image
        userImage.src = "assets/welcome/images/signup_student.webp";
    }
});
            ...
            case 'student':
                ...
                fieldsHtml = `
                    ...
                    <div class="form-group mt-15">
                        ...
                        <input type="text" id="school-location" name="school_location" class="form-control" required>
                    </div>
                `;
                break;
            case 'teacher':
